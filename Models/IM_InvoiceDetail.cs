//------------------------------------------------------------------------------
// <auto-generated>
//     此代码已从模板生成。
//
//     手动更改此文件可能导致应用程序出现意外的行为。
//     如果重新生成代码，将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class IM_InvoiceDetail
    {
        public Nullable<int> id { get; set; }
        public string guids { get; set; }
        public string cciqcode { get; set; }
        public int autoid { get; set; }
        public string cinvcode { get; set; }
        public string cExpCode { get; set; }
        public string cpucomunitcode { get; set; }
        public Nullable<decimal> fchangrate { get; set; }
        public Nullable<decimal> fquantity { get; set; }
        public Nullable<decimal> finqty { get; set; }
        public Nullable<decimal> finnum { get; set; }
        public Nullable<decimal> fnum { get; set; }
        public Nullable<decimal> ftaxprice { get; set; }
        public Nullable<decimal> ftaxmoney { get; set; }
        public Nullable<decimal> fnattaxprice { get; set; }
        public Nullable<decimal> fnattaxmoney { get; set; }
        public Nullable<decimal> fprice { get; set; }
        public Nullable<decimal> fmoney { get; set; }
        public Nullable<decimal> fnatprice { get; set; }
        public Nullable<decimal> fnatmoney { get; set; }
        public Nullable<decimal> ftax { get; set; }
        public Nullable<decimal> fnattax { get; set; }
        public Nullable<decimal> fpaymoney { get; set; }
        public Nullable<decimal> fnatpaymoney { get; set; }
        public Nullable<System.DateTime> dcalcdate { get; set; }
        public string citem_class { get; set; }
        public string citem_cname { get; set; }
        public string citemcode { get; set; }
        public string citemname { get; set; }
        public string objectid_source { get; set; }
        public Nullable<int> autoid_source { get; set; }
        public string ufts_source { get; set; }
        public Nullable<int> irowno_source { get; set; }
        public string cdefine22 { get; set; }
        public string cdefine23 { get; set; }
        public string cdefine24 { get; set; }
        public string cdefine25 { get; set; }
        public Nullable<decimal> cdefine26 { get; set; }
        public Nullable<decimal> cdefine27 { get; set; }
        public string cdefine28 { get; set; }
        public string cdefine29 { get; set; }
        public string cdefine30 { get; set; }
        public string cdefine31 { get; set; }
        public string cdefine32 { get; set; }
        public string cdefine33 { get; set; }
        public Nullable<int> cdefine34 { get; set; }
        public Nullable<int> cdefine35 { get; set; }
        public Nullable<System.DateTime> cdefine36 { get; set; }
        public Nullable<System.DateTime> cdefine37 { get; set; }
        public string cfree1 { get; set; }
        public string cfree2 { get; set; }
        public string cfree3 { get; set; }
        public string cfree4 { get; set; }
        public string cfree5 { get; set; }
        public string cfree6 { get; set; }
        public string cfree7 { get; set; }
        public string cfree8 { get; set; }
        public string cfree9 { get; set; }
        public string cfree10 { get; set; }
        public Nullable<decimal> ftaxrate { get; set; }
        public Nullable<decimal> foverflowrange { get; set; }
        public string cnlostcode { get; set; }
        public Nullable<decimal> ilostquan { get; set; }
        public Nullable<decimal> mnlosttax { get; set; }
        public Nullable<decimal> inlostquan { get; set; }
        public Nullable<decimal> itaxrate { get; set; }
        public Nullable<decimal> inlostMoney { get; set; }
        public string cordercode { get; set; }
        public Nullable<int> iorderrowno { get; set; }
        public Nullable<int> iorderautoid { get; set; }
        public Nullable<int> sotypeid { get; set; }
        public string csocode { get; set; }
        public Nullable<int> isorowno { get; set; }
        public string sodid { get; set; }
        public string ccredence { get; set; }
        public Nullable<bool> bGeneFollowVoucherDetail { get; set; }
        public string cveninvcode { get; set; }
        public Nullable<int> iorderdid { get; set; }
        public Nullable<int> iordertype { get; set; }
        public string csoordercode { get; set; }
        public Nullable<int> iorderseq { get; set; }
        public Nullable<int> iciqbookid { get; set; }
        public string cciqbookcode { get; set; }
        public Nullable<decimal> fciqchangrate { get; set; }
        public string cPZNum { get; set; }
        public Nullable<System.DateTime> doutbilldate { get; set; }
        public Nullable<System.DateTime> dkeepdate { get; set; }
        public string ccontractid { get; set; }
        public string ccontracttagcode { get; set; }
        public Nullable<System.Guid> ccontractrowguid { get; set; }
        public Nullable<decimal> fnatmoneysettled { get; set; }
        public Nullable<decimal> fcuspaymoney { get; set; }
        public Nullable<decimal> fnatcuspaymoney { get; set; }
        public Nullable<decimal> fnatmoneyexcise { get; set; }
        public Nullable<decimal> fnatmoneyaddtax { get; set; }
        public Nullable<decimal> fnatmoneyptax { get; set; }
        public Nullable<decimal> fantmoneyimpost { get; set; }
        public Nullable<decimal> fexciserate { get; set; }
        public Nullable<decimal> faddtaxrate { get; set; }
        public string cexch_namel { get; set; }
        public Nullable<decimal> fexchratel { get; set; }
        public Nullable<decimal> fcarriage { get; set; }
        public Nullable<decimal> finsurance { get; set; }
        public Nullable<decimal> fotherfee { get; set; }
        public string cinvoiceno { get; set; }
        public Nullable<int> idecl { get; set; }
        public Nullable<bool> bcall { get; set; }
        public string cbsysbarcode { get; set; }
        public string cbmemo { get; set; }
    }
}
